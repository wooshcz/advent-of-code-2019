...
comp = program.IntcodeComputer(intcode)

ret = comp.run()
#print(ret)
#print(comp.isWaitingForInput())
...
orient = (0,1)
colors = { pos: 1 }
running = comp.isRunning
while running == True:
  #print(colors)
  if comp.isWaitingForInput() == True:
    ...
    if colors[pos] == 0:
      ...
      inp = 1
    print("Current input: "+str(inp))
    comp.writeInput(inp)
    (paint, turn) = comp.readBuffer()
    print("Paint as: "+str(paint))
    print("Turn: "+str(turn))
    colors[pos] = paint
    # turn left
    if turn == 0:
      orient = (
        round(math.cos(math.atan2(orient[1],orient[0])+math.pi/2), 0), 
        round(math.sin(math.atan2(orient[1],orient[0])+math.pi/2), 0)
        )
    # turn right
    elif turn == 1:
      orient = (
    ...
    print(orient)
    pos = (int(pos[0]+orient[0]), int(pos[1]+orient[1]))
    print(pos)
  running = comp.isRunning
...
lines = []
for iy in range(min(lsy)-1, max(lsy)+2):
  line = ""
  for ix in range(min(lsx)-1, max(lsx)+2):
    key = (ix, iy)
    if colors.get(key) == None:
      line += " "
    elif colors.get(key) == 0:
      line += " "
