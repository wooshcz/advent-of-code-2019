...
def constructEdges(wire):
  ...
  for inst in wire:
    ...
    if letter == 'U':
      ...
      oldy = y
      y = y+length
    edges.append((oldx,oldy,x,y))
  return edges

def intersections(edges1, edges2):
  out = []
  for tupl1 in edges1:
    xmax1 = max(tupl1[2], tupl1[0])
    xmin1 = min(tupl1[2], tupl1[0])
    ymax1 = max(tupl1[3], tupl1[1])
    ymin1 = min(tupl1[3], tupl1[1])
    for tupl2 in edges2:
      xmax2 = max(tupl2[2], tupl2[0])
      xmin2 = min(tupl2[2], tupl2[0])
      ymax2 = max(tupl2[3], tupl2[1])
      ymin2 = min(tupl2[3], tupl2[1])
      if xmax2 > xmax1 and xmin2 < xmin1 and ymax1 > ymax2 and ymin2 > ymin1:
        coord = (xmax1, ymax2)
        out.append(coord)
      if xmax1 > xmax2 and xmin1 < xmin2 and ymax2 > ymax1 and ymin1 > ymin2:
        coord = (xmax2, ymax1)
        out.append(coord)
  return out

edges1 = constructEdges(wire1)
edges2 = constructEdges(wire2)

#print(edges1)

ints = intersections(edges1, edges2)

print(sorted(ints, key=lambda tup: abs(tup[0])+abs(tup[1])))
